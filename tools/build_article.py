...
def build(num,date,publish=False):
    ...
    if re.search(r'<!--\s*要確認',body): sys.exit('ERROR: 本文に「要確認」が残っています。解消してから公開してください。')
    ...
    if publish:
        ...
        if f'blog/{slug}.html' not in s:
            card=f'\n      <a class="col-card" href="blog/{slug}.html"><img src="images/{lead}" alt="" loading="lazy"><span><small>{date}　{html.escape(cat)}</small><b>{html.escape(title)}</b><em>{html.escape(desc[:60])}…</em></span></a>'
